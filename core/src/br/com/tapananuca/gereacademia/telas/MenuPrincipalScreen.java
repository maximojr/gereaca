...
public class MenuPrincipalScreen extends Tela {
	...
	private final Pixmap pm1, pm2;
	private final Drawable cinza, cinzaEscuro;
	...
	public MenuPrincipalScreen(GereAcademia applicationListener) {
		...
		pm2.setColor(Color.WHITE.r, Color.WHITE.g, Color.WHITE.b, 0.2f);
		pm2.fill();
		cinzaEscuro = new TextureRegionDrawable(new TextureRegion(new Texture(pm2)));
		...
		final Container<Actor> containerPagamentos = new Container<Actor>();
		containerPagamentos.setBackground(cinzaEscuro);
		...
		final Container<Actor> containerAniv = new Container<Actor>();
		containerAniv.setBackground(cinzaEscuro);
		containerAniv.align(Align.top);
	...
	public void carregarTabelasMenuPrincipal(int pagina, String dataRef) {
		...
		this.datasRefAniversario.setSelected(MesUtil.getEnumByCodigo(mes).getNome());
		
		if (mes == 1){
			this.carregarAniversariantes(pagina, MesUtil.getEnumByCodigo(mes));
		}
	}
